...
import mobilebanking.feature.transactionsuccess.generated.resources.Res
...
//            }
            ...
            val path = SharedRes.getRes("files/success.json")
            jsonString = Res.readBytes(path).decodeToString()
            AppLogger.i("Lottie json", "$jsonString")

        } catch (e: Exception) {
            e.printStackTrace()
            jsonString = getFallbackAnimation()
        } finally {
            loading = false
        }
    }
